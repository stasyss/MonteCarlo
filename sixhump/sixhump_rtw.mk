...
MODEL                = sixhump
MODULES              = sixhump_initialize.cpp sixhump_terminate.cpp sixhump.cpp rt_nonfinite.cpp rtGetNaN.cpp rtGetInf.cpp 
MAKEFILE             = sixhump_rtw.mk
...
MODELLIB                  = sixhump.lib
MODELREF_LINK_LIBS        = 
MODELREF_LINK_RSPFILE     = sixhump_ref.rsp
MODELREF_INC_PATH         = 
RELATIVE_PATH_TO_ANCHOR   = D:\DOKUME~1\Studijos\MAGIST~2\IIKURS~1\PAULAV~1\MATLAB~1
...
MATLAB_INCLUDES = $(MATLAB_INCLUDES);D:\DOKUME~1\Studijos\MAGIST~2\IIKURS~1\PAULAV~1\MATLAB~1
...
DLLFLAGS = $(ldebug) $(conflags) $(ADDITIONAL_LDFLAGS)
...
!elseif "$(MODELREF_TARGET_TYPE)" == "rtw:dll"
#--- Shared library target ---
$(PRODUCT) : $(OBJS) $(SHARED_LIB)
	@cmd /C "echo ### Linking ..."
	$(PERL) $(GEN_LNK_SCRIPT) $(CMD_FILE) $(OBJS)
	$(LD) $(DLLFLAGS) /DLL /MANIFEST /DEF:$(MODEL).def /OUT:$(PRODUCT) @$(CMD_FILE) $(S_FUNCTIONS_LIB)
	mt -outputresource:"$(PRODUCT);2" -manifest "$(PRODUCT).manifest"
	@cmd /C "echo $(BUILD_SUCCESS) shared library $(MODELLIB)"
!else
